...
- **完全自定義導航**：`store.js` 支援各知識庫自定義選單名稱、導航分組、主題色以及**專屬功能分頁**。
- **資料隔離**：各個知識庫的學習進度、日誌記錄與資料內容均實現物理或邏輯上的隔離。

### 1.2 AI 專屬功能：情報與實驗
針對 AI 知識庫，系統引入了兩大核心工具：
- **動態 RSS 新聞訂閱**：透過 `rssFetcher.js` 獲取 OpenAI, MIT 等權威來源的最新技術動態。
- **提示詞實驗室 (Prompt Lab)**：提供模擬環境，讓使用者理解 Temperature 與模式選擇對模型輸出的影響。
